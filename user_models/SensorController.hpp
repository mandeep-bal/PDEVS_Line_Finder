...
public:
    ...
    std::vector<MSG> out() const noexcept {
    	switch (_state){
    		...
    		//		ORIGINAL
    				if(sensor_input == SPEED) //ADDED
    				    output_val = ON_TRACK2;
    				else if(sensor_input == DARK)
    					output_val = ON_TRACK;
    				else if(sensor_input == BRIGHT)
    					output_val = OFF_TRACK;
    				else if(sensor_input == DARK_RIGHT) //ADDED
    				    output_val = OFF_TRACK2;
    				else if(sensor_input == DARK_LEFT) //ADDED
    				    output_val = OFF_TRACK3;

    				//MODIFIED
    //				if(sensor_input == BRIGHT)
    //			    output_val = ON_TRACK;
   	//			else if(sensor_input == DARK)
  	//			    output_val = OFF_TRACK;
 	//			else if(sensor_input == DARK_LEFT)
	//				output_val = OFF_TRACK;
 	//			else if(sensor_input == DARK_RIGHT)
 	//			    output_val = OFF_TRACK2;
 	//			else if(sensor_input == ALL_DARK)
 	//			    output_val = OFF_TRACK3;
